...
```{r}
renderPlotly({
plot_ly(data = scot_data, x = ~date, y = ~confirmed_cases, color = ~I("#297fe1"), type = "scatter", mode = "lines", 
...
```{r}
renderPlotly({
plot_ly(data = data_testing_long, x = ~date, y = ~Number, text = ~`Daily NHS_RTC`, color = ~Result, type = "bar", colors = c("#297fe1", "#E69F00"),
#colors = c("#b24b29", "#297fe1"),
               hovertemplate = paste('Date: %{x}',
                        '<br>Number: %{y}',
                        '<br>Total tests: %{text}<extra></extra>')) %>%
  plotly::config(displayModeBar = FALSE)  %>%
  layout(barmode = 'stack',
         xaxis = list(
        type = 'date',
...
<!---### Cumulative confirmed cases per test conducted\*
```{r}
renderPlotly({
plot_ly(data = scot_tests, x = ~date, y = ~cases_per_test, color = ~I("#297fe1"), type = "scatter", mode = "lines", 
               hovertemplate = paste('Date: %{x}',
...
### Daily % of people testing positive out of total number of people tested
```{r}
renderPlotly({
fig <- scot_tests_perc %>%
  ...
  plot_ly(data = ., x = ~date, y = ~positive_perc, type = "bar", color = I("#297fe1"),
          name = "Daily")
...
Row  {data-height=100}
-----------------------------------------------------------------------
<font size="2" color="#ff0000">Please note that the daily number of new positive cases up until 14/06/2020 are based on tests carried out through NHS Scotland labs only. From 15/06/2020 people tested through the UK Government (UKG) testing programme (Regional Testing Centres, Mobile Testing Units and home testing kits) testing programme are also included. Previous data have not been changed to account for the UKG data so the total of the daily numbers is 2,275 fewer than the cumulative total. Please note that the **increase in the cumulative figures** on 15 June is due to the addition of the backlog of UKG cases to the database. [More info..](https://www.gov.scot/publications/coronavirus-covid-19-trends-in-daily-data/)</font>
...
renderPlotly({
  ...
  #        yaxis = list(title = "",
  #                     tickformat = "digit"),
  #         legend = list(x = 0.02, y = 0.9)
  #        ) %>%
  #   partial_bundle()
  
  plot_ly(data = data_hosp_hosp, x = ~date, y = ~Hospital_confirmed, text = ~Hospital_confirmed, 
        color = I("#440154FF"), type = "bar", 
        hovertemplate = paste('Date: %{x}',
                              '<br>Hospital confirmed: %{y}<extra></extra>')) %>%
  plotly::config(displayModeBar = FALSE)  %>%
  layout(barmode = 'stack',
         xaxis = list(
           type = 'date',
...
plot_ly(data = data_hosp_icu, x = ~date, y = ~ICU_confirmed, text = ~ICU_confirmed, 
        color = I("#21908CFF"), type = "bar", 
        hovertemplate = paste('Date: %{x}',
                              '<br>ICU confirmed: %{y}<extra></extra>')) %>%
  plotly::config(displayModeBar = FALSE)  %>%
  layout(barmode = 'stack',
         xaxis = list(
           type = 'date',
...
Cases and Maps {data-navmenu="Health Boards"}
=======================================================================

Row {data-height=485}
-------------------------------------

### Cumulative confirmed cases by Health Board as of `r format(max(ymd(scot_data_raw$date)), '%d %B %Y')`
...
line_2 <- tibble( days = seq(0, max(ts$days)-18)) %>% 
  mutate(cases = c(ref, ref * exp(log(2) / 2) ^ (1:(n()-1))))

line_3 <- tibble( days = seq(0, max(ts$days)-95)) %>% 
  mutate(cases = c(ref, ref * 1.260 ^ (1:(n()-1))))

line_7 <- tibble( days = seq(0, max(ts$days)-45)) %>% 
  mutate(cases = c(ref, ref * 1.104 ^ (1:(n()-1))))

line_14 <- tibble( days = seq(0, max(ts$days)-10)) %>% 
  mutate(cases = c(ref, ref * exp(log(2) / 14) ^ (1:(n()-1))))
...
p<- ts %>% 
  ...
  scale_y_continuous(trans = "log10",
                     breaks = c(1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 5000, 50000),
  ...
  geom_line(data = line_14, aes(x = days, y = cases), colour = "grey30", lty = "twodash") +
...
w %>%
  ...
  style(text = "Doubling in 14 days", traces = 16) 

})
```


### Confirmed cases/deaths and rates per 10,000 of population by Health Board in Scotland
```{r}
renderLeaflet({
  ...
  #breaks <- unique(breaks)
  breaks <- c(breaks[1]-0.01, breaks[2:4], breaks[5]+0.01)
  ...
  breaks <- c(breaks[1], breaks[2:4], breaks[5]+0.01)
  breaks <- unique(breaks)
  ...
  leaflet(data = rmapshaper::ms_simplify(cases_by_area, keep = 0.15)) %>%
    ...
    addTiles(urlTemplate = "", attribution = str_glue("Copyright Scottish Government, contains Ordnance Survey data © Crown copyright and database right (2019) | Data: cases: HPS upto ",{as.character(max(scot_data$date))},", deaths: NRS upto ", {as.character(nrs_week)}), options = providerTileOptions(minZoom = 5, maxZoom = 9)) %>%
...
<font size="2" color="#ff0000"> Please note that as of 15 June, cumulative cases includes people tested through the UK Government (UKG) testing programme (Regional Testing Centres, Mobile Testing Units and home testing kits). Prior to 15 June the figures show people tested through NHS labs only. The increase on 15 June is due to the addition of all the UKG cases to the database. [More info..](https://www.gov.scot/publications/coronavirus-covid-19-trends-in-daily-data/)</font>

Cases Trajectories {data-navmenu="Health Boards"}
=======================================================================

Row 
-------------------------------------
### <strong>Trajectories of COVID-19.</strong></br>This is a plot of new cases against total cases on a log-log scale to emphasise the exponential trajectiories of outbreaks in each Health Board and highlight when these begin to slow down. Please note that as of 15 June, cumulative cases includes people tested through the UK Government (UKG) testing programme (Regional Testing Centres, Mobile Testing Units and home testing kits). Prior to 15 June the figures show people tested through NHS labs only. <strong>The increase on 15 June</strong> (black circlet) is due to the addition of all the UKG cases to the database. [More info..](https://www.gov.scot/publications/coronavirus-covid-19-trends-in-daily-data/).
...
server <- function(input, output) {
  ...
  mutate(health_board = fct_relevel(health_board, "Grand Total", after = Inf)) %>% 
  select(date, health_board, confirmed_cases) %>% 
  group_by(health_board) %>% 
  arrange(date) %>%
  ...
  ggplot(df) +
    ...
    geom_point(data = filter(df_hi, date == ymd("2020-06-15")), aes(x = outcome, y = change), size = 10, shape = 1) +
    scale_x_continuous(trans = "log10",
                       labels = scales::comma_format(accuracy = 1),
                       breaks = breaks,
                       expand = expansion(mult = 0.2),
                       limits = c(NA, max(dat$confirmed_cases))) +
    scale_y_continuous(trans = "log10",
                       labels = scales::comma_format(accuracy = 1),
                       breaks = breaks,
  ...
  possible_countries <- sort(unique(dat$health_board))

  output$cboxes <- renderUI({
    selectizeInput(
      inputId = "countries",
      label = "Choose Health Board (autocompletes + can click and delete)",
      choices = possible_countries,
      selected = c("Grand Total", "Greater Glasgow and Clyde", "Lothian", "Borders", "Fife", "Grampian", "Tayside"),
      multiple = TRUE
    )
  })
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sliderInput("smooth", label = "Smooth over X number of days",
                min = 1, max = 7,
                value = 7, step = 1),
...
Weekly deaths {data-navmenu="Health Boards"}
=======================================================================

Row
-------------------------------------

### Weekly deaths trends upto week commencing `r last_week` (source: NRS)
```{r}
renderPlotly({
plot_ly(data = nrs_deaths_covid_hb %>% filter(week_beginning > dmy("1/3/2020")), x = ~week_beginning, 
        y = ~weekly_deaths_total, text = ~health_board, 
        color = ~health_board, type = 'scatter', mode = 'lines', #colors = c("#b24b29", "#297fe1", "#009E73"), 
        #colors = viridis(3, option = "D")[c(2,1,3)],
...
### Deaths per 10,000 people upto week commencing `r last_week` (source: NRS)
```{r}
renderPlotly({
  plot_ly(data = nrs_deaths_healthboards_total, x = ~forcats::fct_reorder(health_board,deaths_per_10000), y = ~deaths_per_10000, type = 'bar', marker = list(color = "#21908CFF"),
        hovertemplate = paste('Deaths per 10000: %{y}',
                              '<br>Health board: %{x}<extra></extra>')) %>%
  plotly::config(displayModeBar = FALSE)  %>%
  layout(barmode = 'stack',
         xaxis = list(
           title = ""), 
...
Deaths by location {data-navmenu="Health Boards"}
=======================================================================
Row
-------------------------------------

### Deaths by location upto week commencing `r last_week` (source: NRS)

```{r}
renderPlotly({
  nrs_deaths_hb_location %>%
    mutate(Location = factor(Location, levels = c("Hospital", "Care home", "Home / Non-institution", "Other institution"))) %>%
    plot_ly(x = ~forcats::fct_reorder(health_board, Total), 
            y = ~deaths, text = ~Location, 
            color = ~Location, type = "bar", #colors = c("#b24b29", "#297fe1", "#009E73"), 
            colors = viridis(4, option = "D")[c(2,1,3,4)],
            hovertemplate = paste('Heatlh board: %{x}',
...
p <- ggplot(ts) +
  ...
  geom_line(data = line_7, aes(x = days, y = cases), colour = "grey70", lty = 2) +
  geom_line(data = line_14, aes(x = days, y = cases), colour = "grey70", linetype = "twodash") +
  theme(legend.position = "none") +
  theme_bw(base_size = 12) +
  theme(legend.position = "none",
...
w %>%
  ...
  style(text = "Doubling in 3 days", traces = 6) %>%
  style(text = "Doubling in 7 days", traces = 7) %>%
  style(text = "Doubling in 14 days", traces = 8)
})
```

### Total deaths per cumulative confirmed cases\*
```{r}
renderPlotly({
...
### Scotland Covid-19 related deaths data upto week commencing `r last_week` (source: NRS)
```{r}
renderPlotly({
...
### Scotland Covid-19 related deaths by 10,000 population (source: NRS)
```{r}
renderPlotly({
...
Deaths by location {data-navmenu="Scotland"}
=======================================================================
Row
-------------------------------------

### Scotland weekly deaths data upto week commencing `r last_week` (source: NRS)

```{r}
renderPlotly({
plot_ly(data = nrs_deaths_covid_week_location %>% filter(Location != "All" & week_beginning > ymd("2020-03-09")), x = ~week_beginning_f, 
        y = ~weekly_deaths_total, text = ~Location, 
        color = ~Location, type = "bar", #colors = c("#b24b29", "#297fe1", "#009E73"), 
        colors = viridis(3, option = "D")[c(2,1,3)],
        hovertemplate = paste('Week beginning: %{x}',
                              '<br>Number: %{y}',
...
### Scotland weekly deaths trends upto week commencing `r last_week` (source: NRS)
```{r}
renderPlotly({
plot_ly(data = nrs_deaths_covid_week_location %>% filter(Location != "All" & week_beginning > ymd("2020-03-09")), x = ~week_beginning_f, 
        y = ~weekly_deaths_total, text = ~Location, 
        color = ~Location, type = 'scatter', mode = 'lines', #colors = c("#b24b29", "#297fe1", "#009E73"), 
        colors = viridis(3, option = "D")[c(2,1,3)],
        hovertemplate = paste('Week beginning: %{x}',
                              '<br>Number: %{y}',
                              '<br>Total: %{text}<extra></extra>')) %>%
...
renderPlotly({
uk_scot_data %>% 
    mutate(country_region = recode(country_region, "UK" = "UK total")) %>%
...
```{r}
renderPlotly({
  uk_scot_data %>% filter(date >= "2020-01-31") %>%
  mutate(country_region = recode(country_region, "UK" = "UK total")) %>%
  plot_ly(x = ~date, y = ~confirmed_cases, text = ~country_region, color = ~country_region, type = "scatter", mode = "lines+markers", colors = c("#297fe1", "#b24b29"), 
               hovertemplate = paste('Date: %{x}',
                        '<br>Total cases: %{y}',
...
#date of first uk death
  uk_scot_data %>% filter(date >= "2020-03-06") %>%
  mutate(country_region = recode(country_region, "UK" = "UK total")) %>%
plot_ly(x = ~date, y = ~deaths, text = ~country_region, color = ~country_region, type = "scatter", mode = "lines+markers", colors = c("#297fe1", "#b24b29"), 
               hovertemplate = paste('Date: %{x}',
                        '<br>Total deaths: %{y}',
                        '<br>Region: %{text}<extra></extra>')) %>%
  plotly::config(displayModeBar = FALSE)  %>%
...
4. [**Scottish Daily Data (source A)**](https://github.com/watty62/Scot_covid19/blob/master/data/processed) and [**Scottish Daily Data (source B)**](https://www.gov.scot/publications/trends-in-number-of-people-in-hospital-with-confirmed-or-suspected-covid-19/)

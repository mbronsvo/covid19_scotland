...
```{r global_prev, include=FALSE, eval = FALSE}
# Load packages
...
source("R/log_time_traj.R")


# Map files
# SCOTLAND MAP
cases_by_area <- sf::st_read("SG_NHS_HealthBoards_2019c.geojson") %>%
...
```{r global, include=FALSE, eval = TRUE}
library(flexdashboard) ; library(shiny) ; library(readr); library(dplyr); library(tidyr); library(purrr); library(forcats); library(stringr); library(htmlwidgets); library(lubridate); library(sf); library(RcppRoll); library(plotly); library(shinythemes);library(leaflet); library(classInt); library(ggrepel); library(scales); library(leaflet.extras);
...
Cases {data-navmenu="Cases"}
=======================================================================

Row {data-height=10, .small-box}
-----------------------------------------------------------------------
...
```{r}
renderPlotly({
  # ggplot(scot_data, aes(x = date, y = new_cases)) +
  # geom_col(fill = "#297fe1") +
...
renderPlotly({
  ...
  # ggplot(scot_data, aes(x = date, y = confirmed_cases)) +
  # geom_line(colour = "#297fe1", size = 1) +
  # geom_point(colour = "#297fe1", size = 2) +
  ...
  #       panel.grid.minor = element_blank(),
  #       plot.caption = element_text(size = 10, color = "grey50", hjust = 1, margin = margin(t = 15))) +
  #   expand_limits(y = c(0,max(scot_data$confirmed_cases)*1.01)) 
...
Hospital data {data-navmenu="Cases"}
=======================================================================
...
```{r}
renderPlotly({
  # ggplot(scot_deaths, aes(x = date, y = new_deaths, #text = paste0("", comma(Deaths), " deaths", format(Date, "%d %B"))
  #                              )) +
  #   geom_hline(yintercept = 0, size = 1, colour = "#333333") +
  #   geom_col(fill = "#297fe1") +
  #   scale_x_date(expand = c(0.005, 0.005), date_labels = "%d-%b") +
...
```{r}
renderPlotly({
  # ggplot(scot_deaths, aes(x = date, y = deaths, group = 1)) +
  #   geom_hline(yintercept = 0, size = 1, colour = "#333333") +
  #   geom_line(colour = "#297fe1", size = 0.5) +
  #   geom_point(shape = 21, colour = "white", fill = "#297fe1", size = 2.5) +
  #   scale_x_date(expand = c(0.005, 0.1), date_labels = "%d-%b") +
  #   scale_y_continuous(expand = c(0.005, 0.005), labels = comma) +
  #   labs(x = NULL, y = NULL, title = NULL, subtitle = NULL, caption = "Source: github/watty62") +
  #   theme_minimal(base_size = 12) +
  ...
  #         panel.grid.minor = element_blank(),
  #         plot.caption = element_text(size = 10, color = "grey50", hjust = 1, margin = margin(t = 15))) +
  #     expand_limits(y = c(0, max(scot_deaths$deaths)*1.01))
...
Demographics {data-navmenu="Deaths"} 
=======================================================================
Row
-------------------------------------
...
```{r}
renderPlotly({
  # ggplot(uk_scot_data %>% filter(date >= "2020-01-31"), aes(x = date, y = new_cases, fill = country_region)) +
  # geom_col() +
  # geom_hline(yintercept = 0, size = 1, colour = "#333333") +
  #   scale_fill_manual(values = c("#297fe1", "red")) +
  # scale_x_date(expand = c(0.005, 0.005), date_labels = "%d-%b") +
  # scale_y_continuous(expand = c(0.005, 0.005), labels = comma) +
  # labs(x = NULL, y = NULL, title = NULL, subtitle = NULL,
  #      caption = "Source: github/watty62") +
...
# ggplot(uk_scot_data %>% filter(date >= "2020-01-31"), aes(x = date, y = confirmed_cases, group = country_region,
#                                   fill = country_region, colour = country_region)) +
#   geom_line( size = 0.5) +
#     geom_point(shape = 21, colour = "white", size = 2.5) +
#     scale_fill_manual(values = c("#297fe1", "#b24b29")) +
#     scale_colour_manual(values = c("#297fe1", "#b24b29")) +
#   scale_y_continuous(labels = comma, expand = c(0.005, 0.005), sec.axis = sec_axis(~ ., breaks = max(scot_data$confirmed_cases))) +
#   scale_x_date(expand = c(0.005, 0.1), date_labels = "%d-%b") +
#   geom_hline(yintercept = 0, size = 0.5, colour = "#212121") +
#   labs(x = NULL, y = NULL, title = NULL, subtitle = NULL,
#        caption = "Source: github/watty62") +
...
#         panel.grid.minor = element_blank(),
#         plot.caption = element_text(size = 10, color = "grey50", hjust = 1, margin = margin(t = 15))) +
#     expand_limits(y = c(0,max(uk_scot_data$confirmed_cases)*1.01)) +
#   theme(legend.position = "none")
...
renderPlotly({
  ...
  # ggplot(uk_scot_data %>% filter(date >= "2020-03-06"), alpha = 0.5, aes(x = date, y = new_deaths, fill = country_region)) +
  #   geom_hline(yintercept = 0, size = 1, colour = "#333333") +
  #   geom_col() +
  #   scale_fill_manual(values = c("#297fe1", "#b24b29")) +
  #   scale_x_date(expand = c(0.005, 0.005), date_labels = "%d-%b") +
...
renderPlotly({
  #date of first uk death
  # ggplot(uk_scot_data %>% filter(date >= "2020-03-06"), aes(x = date, y = deaths, group = country_region,
  #                                 fill = country_region, colour = country_region)) +
  #   geom_hline(yintercept = 0, size = 1, colour = "#333333") +
  #   geom_line( size = 0.5) +
  #   geom_point(shape = 21, colour = "white", size = 2.5) +
  ...
  #         panel.grid.minor = element_blank(),
  #         plot.caption = element_text(size = 10, color = "grey50", hjust = 1, margin = margin(t = 15))) +
  #     expand_limits(y = c(0, max(uk_scot_data$deaths)*1.01)) +
  # theme(legend.position = "none")
...
server <- function(input, output) {
  ...
  output$cboxes <- renderUI({
    selectizeInput(
      inputId = "countries",
      label = "Choose countries (autocompletes + can click and delete)",
...
3. **UK Regional data**
https://github.com/tomwhite/covid-19-uk-data/tree/master/data </br>
...
7. **World data** https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/ and https://covid.ourworldindata.org/data/ecdc/full_data.csv
<!-- 7. **Thumbnail** https://www.newscientist.com/term/covid-19/ -->
